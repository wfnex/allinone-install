...
function install_rpm() {
	...
	if [ $DISTRIB_ID == "CentOS" ]; then
		...
		#check starcontroller
		if [ -d /opt/starcontroller ];then
			echo starcontroller check!
		else
			if [ -f ../package/centos/starcontroller-1.0.0-1.x86_64.rpm ];then
				rpm -ihv ../package/centos/starcontroller-1.0.0-1.x86_64.rpm
			else
				echo no starcontroller rpm!
			fi
		fi
	else
