...
function install_rpm() {
	...
	if [ $DISTRIB_ID == "CentOS" ]; then
		...
		#check wfnos
		if [ -d /opt/staros.xyz/wfnos ];then
			echo WFNOS check!
		else
			if [ -f ../package/centos/wfnos-$WFNOS_VERSION-1.x86_64.rpm ];then
				rpm -ihv ../package/centos/wfnos-$WFNOS_VERSION-1.x86_64.rpm
			else
				wget -P ../package/centos/ https://github.com/wfnex/wfnos/raw/master/wfnos-$WFNOS_VERSION-1.x86_64.rpm
				rpm -ihv ../package/centos/wfnos-$WFNOS_VERSION-1.x86_64.rpm
			fi
		fi
	else
